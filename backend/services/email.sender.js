...
const sendMailHelper = async (body) => {
  const fromEmail = process.env.SMTP_USER || body.sender?.email;
  const toEmails = body.to.map(t => t.email).join(", ");
  
  const mailOptions = {
    from: `"BidSphere" <${fromEmail}>`,
    to: toEmails,
  ...
  if (body.attachment && body.attachment.length > 0) {
    mailOptions.attachments = body.attachment.map(att => {
      const item = {
        filename: att.name || "attachment.png",
        content: Buffer.from(att.content, 'base64')
      };
      if (att.name === "qr.png") {
        item.cid = "qrimage@bidsphere";
      }
      return item;
    });
  }
...
const fetch = async (url, options) => {
  if (url === "https://api.brevo.com/v3/smtp/email") {
    ...
    const res = await sendMailHelper(body);
    return {
      json: async () => res
    };
  }
